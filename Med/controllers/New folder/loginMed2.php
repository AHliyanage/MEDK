...
function login(){
	...
	try{
		...
		if(count($result) > 0){
			...
			$query = "SELECT staff_id, first_name, last_name, password ,user_lock FROM staff WHERE user_name = '$username'/* AND password ='$password'*/";
					...
					if($password != $row[3]){
						if(!isset($_SESSION["invaldAttempts"])){
							...
							if($_SESSION["invaldAttempts"] >= 3){
								...
								$lockQueryResult = mysqli_query($conn, $query);
							}

						}

					}else{
						//another if for check whether user lock
						if( $row[4] == 1){
							echo "user locked";
						}else{
