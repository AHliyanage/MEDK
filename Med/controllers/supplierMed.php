...
function loadData(){
	...
	try{
			...
			$conn = getConnection();
			$query = "SELECT supplier_code,first_name,last_name,contact_no,address,email,company FROM supplier";
			...
			if($loginQueryResult == true){
				...
				//start-load products
				$queryP = "SELECT product_name FROM product";

				$productQueryResult = mysqli_query($conn, $queryP);
			
					while ($rowP=mysqli_fetch_array($productQueryResult)){
						$dept_name=$rowP[0];
					}
	
				$response->success = $dept_name;
